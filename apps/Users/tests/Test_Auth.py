...
class UserTests(APITestCase): # noqa
    ...
    @patch('apps.Users.Tasks.Auth_tasks.send_otp_to_user_email')  # Patch the OTP sending function
    def test_confirm_email_valid_otp(self, mock_send_otp):  # type: ignore
        ...
        response = self.client.post(url, {'user_uuid': self.user.uuid, 'otp': 1234}, format='json')
    ...
    def test_confirm_email_invalid_otp(self):  # type: ignore
        ...
        url = reverse('confirm-email')
        response = self.client.post(url, {'user_uuid': self.user.uuid, 'otp': 5678}, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_send_reset_otp(self):  # type: ignore
        """Test sending reset OTP"""
        url = reverse('send-reset-otp')
        response = self.client.post(url, {'email': 'existinguser@example.com'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_send_reset_otp_invalid_user(self):  # type: ignore
        ...
        url = reverse('send-reset-otp')
        response = self.client.post(url, {'email': 'nonexisting@example.com'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_login_valid(self):  # type: ignore
        """Test login with valid credentials"""
        url = '/Auth/Login/'  # Login path is not named, so use hardcoded path
        response = self.client.post(url, {'email': 'existinguser@example.com',
